...
with open("annotated_hits_highres.tsv") as inp:
    cnt = 0
    for line in inp:
        cnt += 1
        if line.strip():
...
with open("nr_hits_highres.tsv","w") as outp:
...
with open("nr_hits_highres.tsv") as inp:
    cnt = 0
    for line in inp:
        cnt += 1
        if line.strip():
            ...
            link = "https://rna.bgsu.edu/rna3dhub/display3D/unitid/{}"\
                   .format(','.join([pdb[-4:].upper()+'|'+x.replace('.','|')[:-1] for x in rs if x != '-']))
...
with open("unique_hits_highres.tsv","w") as outp:
...
with open("unique_topologies_highres.tsv","w") as outp:
    for cnt,line, desc, topo,link,pdb in sorted([(v[1],v[2],k,v[3],v[4],v[5]) for k,v in bag.items()]):
        outp.write(line.strip()+'\t'+str(len(counts[desc][0]))+'\t'+str(len(counts[desc][1]))+'\t'+\
                   topo+'\t'+\
                   ','.join(sorted(counts[desc][0]))+'\t'+','.join(sorted(counts[desc][1]))+'\t'+link+'\n')
    ...
    for line in inp:
        ...
        if line.strip():
            ...
            ut, rfam, nrclass = *hit[:6],hit[6:12],hit[12:18],hit[18],hit[19:24],hit[24:29],\
                                ...
                                hit[51],hit[52],hit[53]
            rmsd = float(rmsd)
            topo = loopsize+'-'+'-'.join(conns)

            if loopsize not in ('5','6'):
                continue
...
with open("penta_hexa_highres.tsv","w") as outp:
    for cnt,line, desc, topo,link,pdb in sorted([(v[1],v[2],k,v[3],v[4],v[5]) for k,v in bag.items()]):
        outp.write(line.strip()+'\t'+str(len(counts[desc][0]))+'\t'+str(len(counts[desc][1]))+'\t'+\
                   topo+'\t'+\
                   ','.join(sorted(counts[desc][0]))+'\t'+','.join(sorted(counts[desc][1]))+'\t'+link+'\n')
